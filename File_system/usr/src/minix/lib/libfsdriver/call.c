...
/* Process a INODEWALKER request from VFS */
int
...
/* Process a DAMAGEINODE request from VFS */
int
fsdriver_dinode(const struct fsdriver * __restrict fdp,
		const message * __restrict m_in, message * __restrict __unused m_out)
{
	//printf("HI from call.c \n");
	ino_t ino_nr;
	int r;
	ino_nr = m_in->m_fs_vfs_lookup.inode;
	if (fdp->fdr_dinode == NULL)
		return ENOSYS;
	 r = fdp->fdr_dinode(ino_nr);
	//printf("HI from call.c \n");
	return r;
}
/* Process a RESTOREINODE request from VFS */
int
fsdriver_rinode(const struct fsdriver * __restrict fdp,
		const message * __restrict m_in, message * __restrict __unused m_out)
{
	//printf("HI from call.c \n");
	ino_t ino_nr;
	int r;
	ino_nr = m_in->m_fs_vfs_lookup.inode;
	if (fdp->fdr_rinode == NULL)
		return ENOSYS;
	 r = fdp->fdr_rinode(ino_nr);
	//printf("HI from call.c \n");
	return r;
}
