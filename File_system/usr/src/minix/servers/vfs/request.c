...
int req_damageinode(endpoint_t fs_e,ino_t inode)
{

  message m;
  /* Fill in request message */
  m.m_type = REQ_DAMAGEINODE;
...
int req_restoreinode(endpoint_t fs_e,ino_t inode)
{
  printf("%llu", inode);
  printf("HI From the new file in request \n");
  message m;
  /* Fill in request message */
  m.m_type = REQ_RESTOREINODE;
  m.m_fs_vfs_lookup.inode = inode;
